<template>
    ...
    <layout :time="time" :user="user" :check="check">
        <div>
            ...
            <h1 class="title">خروجی ثابت ماهیانه</h1>
            <Link class="btn btn-danger back" :href="route('home.index')"> بازگشت</Link>
        </div>
        <Link class="btn btn-success  mr-4 mb-4" :href="route('date.create',user.id)"> ایجاد حساب جدید</Link>
        <table class="table table-dark table-hover">
            ...
            <tr class="text-center">
                ...
                <th scope="col">تاریخ چندم هر ماه</th>
                <th scope="col">تنظیمات</th>
            </tr>
            </thead>
            <tbody>
            <tr v-if="date.length === 0">
                <td colspan="5" class="text-danger text-center">هیچ خروجی ثابتی هنوز توسط شما ثبت نشده است</td>
            </tr>

            <tr class="text-center" v-for="(item , index) in date" >
                <th scope="row">{{index+1}}</th>
                <td>{{item.name}}</td>
                <td>{{item.price}}</td>
                <td>{{item.day}}</td>
                <td><button @click="destroy(item.id)" class="btn btn-danger mr-2">حذف حساب</button></td>
            </tr>
            </tbody>
        </table>
        ...
        setup() {
            const destroy = (id) =>{
                ...
                swalWithBootstrapButtons.fire({
                    title: 'آیا از حذف کردن خروجی ثابت ماهیانه مطمئن هستید؟',
                    text: "یاد آور خروجی ثابت برای این مورد غیر فعال خواهد شد!",
                    icon: 'warning',
