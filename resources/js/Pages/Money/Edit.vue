<template>
    ...
    <layout :time="time" :user="user" :check="check">
        <div class="main">
            ...
            <Link class="btn btn-danger back" :href="route('money.index',[user.id,category.id])"> بازگشت</Link>

            <form @submit.prevent="form.put(route('money.update',form.id))">
...
<script>
    ...
    export default {
        ...
        setup(props){
            const form = useForm({
                id: props.money.id,
                add_money: props.money.add_money,
                minus_money:props.money.minus_money,
                time:'',
                comment: props.money.comment
            });
        ...
        methods: {
            ...
            updateMoney: function () {
                Inertia.post(`/money/update/${props.money.id}`, {
                    _method: 'put',
                    add_money: form.add_money,
                    minus_money: form.minus_money,
                    time: form.time,
