<template>
    ...
    <layout :time="time" :user="user" :check="check">
        <div>
            ...
            <h1 class="title">مشاهده تراکنش ها </h1>
            <Link class="btn btn-danger back" :href="route('category.index',user.id)"> بازگشت</Link>
        </div>
        ...
        <table class="table table-dark table-hover">
            ...
            <tr class="text-center" v-for="(item , index) in items.data" :key="item.id">
                <th scope="row">{{index+1}}</th>
                <td>{{name}}</td>
                <td class="bg-success" v-if="item.add_money">{{item.add_money}}</td>
                <td v-else>---</td>
                <td class="bg-danger" v-if="item.minus_money">{{item.minus_money}}</td>
                <td v-else>---</td>
                <td class="text-center">
                    <Link class="btn btn-warning" :href="route('money.show',[user.id,category.id,item.id])">جزییات تراکنش </Link>
